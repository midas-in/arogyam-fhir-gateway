...
import in.ac.iisc.midas.fhir.gateway.modules.inbound.authorization.enforcer.UnifiedEnforcer;
...
import static in.ac.iisc.midas.fhir.gateway.modules.inbound.shared.AuthenticationOutcome.getOutcome;
...
public class FHIREndpointAuthorizationInterceptor extends AuthorizationInterceptor {
    ...
    private IAuthRuleBuilder attachAccessPermissionRules(Set<UserSessionDetail.GrantedAuthority> authorities, IAuthRuleBuilder ruleBuilder) {
        var permissions  = accessPermissions.stream().map(PermissionGroup::getName).toList();
